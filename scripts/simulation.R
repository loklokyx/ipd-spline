...
CONFIG <- list(
  ...
  noise_sd = 2,
  include_meta_re = FALSE,
  weight_fun = NULL,
  setting_dir = "./data/simulate_coverage1/"
)
...
run_stage1 <- function(sim_data, study_ids, stage1_knots_vec) {
  ...
  for (s_idx in seq_along(study_ids)) {
    ...
    # pred <- predict(model, newdata = study_data)
...
run_single_simulation <- function(sim_i, study_ids, stage1_knots_vec, stage2_knots) {
  ...
  if (CONFIG$include_meta_re) {
    # stage2_results$meta_re <- run_meta_fixed(stage1_results$data, CONFIG$stage2_knots, CONFIG$weight_fun, "random")
    # stage2_results$meta_re <- predict(meta_model, newdata = CONFIG$data, re.form = ~(1 | Study))
  }
