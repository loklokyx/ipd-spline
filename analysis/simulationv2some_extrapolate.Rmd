...
# Read Settings
```{r}
file_dir <- "../data/extrapolate/simulated_some/"
...
summary_all_some %>% arrange(method, rank_id)
```
...
### line plot
```{r, fig.width=10}
plots_some <- lapply(metrics, function(m) base_plot(df_plot_some, m, 
...
wrap_plots(
  list(plots_some[[1]], plots_some[[2]], plots_some[[3]]),
  ncol=3,
)
```
...
### CI True Value
```{r, fig.width=12}
plot_CI_some <- base_plot_CI(df_plot_some, ncol = 2, legend_ncol=4, show_jitter = T)
plot_CI_some
```