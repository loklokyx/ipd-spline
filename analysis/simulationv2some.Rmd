...
rank_all %>% select(-all_of(exclude_cols))
```
...
```
```{r}
plot(Y~Age, col=Study, data=sim_data_some[[1]])
```
...
summary_all <- set_summary(all_metrics_some, metrics = metrics)
summary_all
```
...
### line plot
```{r}
df_all <- bind_rows(all_metrics_some)
df_plot <- df_all %>%
  filter(
    method == rank_method,
    method != "meta_re"
...
plots <- lapply(metrics, function(m) base_plot(df_plot, m))
plots
```
### CI True Value
```{r, fig.height=8, fig.width=8}
plot_CI <- base_plot_CI(df_plot, ncol = 2)
plot_CI
```