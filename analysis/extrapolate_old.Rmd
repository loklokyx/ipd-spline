...
#        x = "Age", y = "Y")
plot_stage1(res_no, knot = s1_knot)
...
## func
```{r}
run_pooled <- function(pred_data, knots = 5, weight_fun = NULL, interval = "") {
  d <- pred_data
  # d$study <- factor(1)
  combine_pointwise(d, weight_fun = weight_fun)
}

run_meta_fixed <- function(pred_data, knots = 5, weight_fun = NULL, interval = "") {
  d <- pred_data
  d <- d %>% mutate(weight = if (!is.null(weight_fun)) weight_fun(SE) else 1)

  pred_str <- make_rcs_formula("Age", make_quantiles(d$Age, s2_knot))
