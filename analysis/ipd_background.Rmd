...
```{r}
combined_data <- run_pooled(pred_ori_data, 5, NULL, "response_s")
```

## Method 2: Meta-analysis using per-study predicted splines
```{r}
meta_data <- pred_ori_data
meta_data <- run_meta_fixed(pred_ori_data, 5, NULL, "random")
```
...
ggplot() +
  geom_point(data = ipd_data, aes(x = Age, y = Y, color = Study), alpha = 0.3) +
  geom_line(data = plot_data, aes(x = X1, y = Y), color = "black", linewidth = 1) +
  geom_line(data = combined_data, aes(x = Age, y = Y),
            color = "blue", linewidth = 1) +
  geom_ribbon(data = combined_data %>% arrange(Age),
            aes(x = Age, ymin = lower, ymax = upper),
            fill = "blue", alpha = 0.2) +
  geom_line(data = meta_data, aes(x = Age, y = Y),
