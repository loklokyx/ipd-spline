...
ggplot(ipd_data) +
  geom_point(aes(x = Age, y = Y, color = Study), alpha = 0.3) +
  geom_line(data=plot_data, aes(x = X1, y = Y), color = "black", linewidth = 1) +
  labs(title = "Included noise of Y~N(0,2)", x = "Age", y = "Y")
```
# Stage 1
...
study_models <- setNames(lapply(studies, function(s) {
  ...
  quan <- make_quantiles(study_data$Age, knot = 4)
...
pred_ori_data <- do.call(rbind, lapply(study_models, function(sm) {
  pred <- predict(sm$model, newdata = sm$data, se.fit = TRUE)
...
pred1 <- predict(pooled_model, newdata = combined_data, se.fit=T)
combined_data$Y_pred <- pred1$linear.predictors

resid_sd <- sqrt(sum(residuals(pooled_model)^2) / pooled_model$df.residual)
se_total <- sqrt(pred1$se.fit^2 + resid_sd^2)

combined_data$lower <- pred1$linear.predictors - 1.96 * se_total
combined_data$upper <- pred1$linear.predictors + 1.96 * se_total
```
### weighted
```{r}
weight_fun <- function(se) 1/se^2   # inverse variance
combined_data_w <- run_pooled(pred_ori_data, 5, weight_fun, "response")
```
...
meta_pred <- predict(meta_model, newdata = meta_data, re.form = NA, se.fit = TRUE)
meta_data$Y_pred <- meta_pred$fit

meta_data$lower <- meta_pred$fit - 1.96 * meta_pred$se.fit
meta_data$upper <- meta_pred$fit + 1.96 * meta_pred$se.fit
```
### weighted
```{r}
meta_data_w <- run_meta_fixed(pred_ori_data, 5, weight_fun)
```
## combine
```{r}
ggplot() +
  geom_point(data = ipd_data, aes(x = Age, y = Y, color = Study), alpha = 0.3) +
  geom_line(data = plot_data, aes(x = X1, y = Y), color = "black", linewidth = 1) +
  geom_line(data = combined_data, aes(x = Age, y = Y_pred),
            color = "blue", linewidth = 1) +
  geom_ribbon(data = combined_data %>% arrange(Age),
            aes(x = Age, ymin = lower, ymax = upper),
            fill = "blue", alpha = 0.2) +
  geom_line(data = meta_data, aes(x = Age, y = Y_pred),
            color = "red", linewidth = 1.0) +
  geom_ribbon(data = meta_data %>% arrange(Age),
            aes(x = Age, ymin = lower, ymax = upper),
            fill = "red", alpha = 0.2) +
  geom_vline(xintercept = make_quantiles(combined_data$Age, knot = 5),
             linetype = "dashed", color = "grey40") +
  labs(
    title = "Stage 2: Pooled(Blue) vs Meta-Analysed(Red)",
    y = "Predicted Y"
  )
...
ggplot() +
  geom_point(data = ipd_data, aes(x = Age, y = Y, color = Study), alpha = 0.3) +
  geom_line(data = plot_data, aes(x = X1, y = Y), color = "black", linewidth = 1) +
  geom_line(data = combined_data_w, aes(x = Age, y = Y),
            color = "blue", linewidth = 1) +
  geom_ribbon(data = combined_data_w,
            aes(x = Age, ymin = lower, ymax = upper),
            fill = "blue", alpha = 0.2) +
  geom_line(data = meta_data_w, aes(x = Age, y = Y),
            color = "red", linewidth = 1.0) +
  geom_ribbon(data = meta_data_w,
            aes(x = Age, ymin = lower, ymax = upper),
            fill = "red", alpha = 0.2) +
  geom_vline(xintercept = make_quantiles(combined_data$Age, knot = 5),
             linetype = "dashed", color = "grey40") +
...
ggplot(meta_data_re) +
  ...
  geom_line(data = plot_data, aes(x = X1, y = Y), color = "black", linewidth = 1) +
  geom_line(aes(x = Age, y = Y_pred, color = study), linewidth = 1) +
  geom_vline(xintercept = make_quantiles(combined_data$Age, knot = 5), linetype = "dashed") +
  labs(
    title = "Stage 2: Meta-Analysed Including Study RE",
    y = "Predicted Y"
  )
```
### weighted
```{r}
meta_data_w_r <- run_meta_fixed(pred_ori_data, 5, weight_fun, "random")

# weighted
ggplot() +
  geom_point(data = ipd_data, aes(x = Age, y = Y, color = Study), alpha = 0.3) +
  geom_line(data = plot_data, aes(x = X1, y = Y), color = "black", linewidth = 1) +
  geom_line(data = meta_data_w_r, aes(x = Age, y = Y),
            color = "red", linewidth = 1.0) +
  geom_ribbon(data = meta_data_w_r,
            aes(x = Age, ymin = lower, ymax = upper),
            fill = "red", alpha = 0.2) +
  geom_vline(xintercept = make_quantiles(combined_data$Age, knot = 5),
             linetype = "dashed", color = "grey40") +
