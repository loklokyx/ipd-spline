...
res_no <- run_ipd_ma(ipd_data_no, noise_sd = 1, knots = s1_knot)
...
res_some <- run_ipd_ma(ipd_data_some, noise_sd = 1, knots = s1_knot)
...
res_many <- run_ipd_ma(ipd_data_many, noise_sd = 1, knots = s1_knot)
...
p_meta_no <- ggplot() +
  ...
  geom_ribbon(data = meta_fixed_no %>% arrange(Age),
              ...
              fill = "red", alpha = 0.2) +
  geom_vline(xintercept = make_quantiles(ipd_data_no$Age, knot = s2_knot), linetype = "dashed") +
...
p_meta_some <- ggplot() +
  ...
  geom_ribbon(data = meta_fixed_some %>% arrange(Age),
              aes(x = Age, ymin = lower, ymax = upper),
              fill = "red", alpha = 0.2) +
  geom_vline(xintercept = make_quantiles(ipd_data_some$Age, knot = s2_knot), linetype = "dashed") +
...
p_meta_many <- ggplot() +
  ...
  geom_ribbon(data = meta_fixed_many %>% arrange(Age),
              aes(x = Age, ymin = lower, ymax = upper),
              fill = "red", alpha = 0.2) +
  geom_vline(xintercept = make_quantiles(ipd_data_many$Age, knot = s2_knot), linetype = "dashed") +
...
ddd <- list(No = d_pred_no, Some = d_pred_some, Many = d_pred_many)
# ddd <- list(d_pred_no)
for (nm in names(ddd)) {
  d <- ddd[[nm]]
  d <- d %>% mutate(w = if (!is.null(weight_fun)) weight_fun(SE) else 1)
  ...
  knot_quan <- make_quantiles(d$Age, knot = s1_knot)
  pp[[i]] <- ggplot(check_sumw, aes(Age, sum_w)) +
    geom_line() +
    geom_vline(xintercept = knot_quan, linetype = "dashed") +
    labs(title = nm)

  pp2[[i]] <- ggplot(check_se, aes(Age, mean_se)) +
    geom_line() +
    geom_vline(xintercept = knot_quan, linetype = "dashed") +
    labs(title = nm)
  i <- i + 1
...
run_meta_fixed <- function(pred_data, weight_fun = NULL) {
  ...
  pred <- predict(meta_model, newdata = d, re.form = NULL, se.fit = TRUE)
  d$Y <- pred$fit
